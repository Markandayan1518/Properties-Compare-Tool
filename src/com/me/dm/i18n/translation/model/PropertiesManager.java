...
public class PropertiesManager {
  ...
  public List<PropertyTableModel.Property> loadProperties(Properties props) {
    return props.entrySet()
        ...
        .sorted((p1, p2) -> {
          return p1.getKey().toString().compareToIgnoreCase(p2.getKey().toString());
        })
    ...
    fromProps.entrySet().stream()
        ...
        .forEach(p -> {
          ...
          if (toValueAsString == null) {
            ...
            this.nonTranslated++;
            result.add(new PropertyTableModel.Property((String) p.getKey(), (String) p.getValue(), toValueAsString,
                com.me.dm.i18n.translation.gui.Action.UNTRANSLATED));
          } else if (!insertedkeys.contains(p.getKey())) {
            insertedkeys.add(p.getKey());
            result.add(
                new PropertyTableModel.Property((String) p.getKey(), fromValueAsString, toValueAsString, com.me.dm.i18n.translation.gui.Action.NONE));
          }
        });
    toProps.entrySet().stream()
        ...
        .forEach(p -> {
          ...
          if (fromValueAsString == null) {
            this.added++;
            result
                .add(new PropertyTableModel.Property((String) p.getKey(), null, (String) p.getValue(), com.me.dm.i18n.translation.gui.Action.ADDED));
          }
        });
